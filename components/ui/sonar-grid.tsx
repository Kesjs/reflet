...
export interface SonarGridProps extends React.ComponentProps<"div"> {
  ...
  baseOpacity?: number
  /** Any CSS color. Defaults to Reflet brand sulfur yellow #c9ab1e. */
  color?: string
 ...
 * Canvas-based and theme-aware, tailored for Reflet AI visibility monitoring.
 */
export function SonarGrid({
  ...
  React.useEffect(() => {
    ...
    let visible = true
    let seeded = false
    let stroke = color || "#c9ab1e"
    let nextPing = performance.now() + opts.current.pingEvery * 1000

    const readColor = () => {
      stroke = color || getComputedStyle(canvas).color || "#c9ab1e"
    }
